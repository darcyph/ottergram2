...
function prev() {
    ...
    for (var i = 0; i < thumbArr.length; i++) {
        if (titleFromThumb(thumbArr[i]) === currTitle.textContent) {
            if (i === 0) {
                currImage = imageFromThumb(thumbArr[thumbArr.length - 1]);
                currTitle = titleFromThumb(thumbArr[thumbArr.length - 1]);
                setDetails(currImage, currTitle);
            } else {
                currImage = imageFromThumb(thumbArr[i-1]);
                currTitle = titleFromThumb(thumbArr[i-1]);
                setDetails(currImage, currTitle);
            }
        }
    }
...
function next() {
    ...
    for (var i = 0; i < thumbArr.length; i++) {
        if (titleFromThumb(thumbArr[i]) === currTitle.textContent) {
            if (i === thumbArr.length - 1) {
                currImage = imageFromThumb(thumbArr[0]);
                currTitle = titleFromThumb(thumbArr[0]);
                setDetails(currImage, currTitle)
            } else {
                currImage = imageFromThumb(thumbArr[i+1]);
                currTitle = titleFromThumb(thumbArr[i+1]);
